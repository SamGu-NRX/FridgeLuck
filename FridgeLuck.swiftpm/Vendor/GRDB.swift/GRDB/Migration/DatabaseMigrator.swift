...
public struct DatabaseMigrator: Sendable {
  ...
  private func unappliedExecutions(upTo targetIdentifier: String, appliedIdentifiers: Set<String>)
    -> [Execution]
  {
    ...
    return expectedMigrations.compactMap { migration in
      if appliedIdentifiers.contains(migration.identifier) {
        ...
        return Execution(
          migration: migration, mode: .run(mergedIdentifiers: appliedMergedIdentifiers))
      }
    }
  }
