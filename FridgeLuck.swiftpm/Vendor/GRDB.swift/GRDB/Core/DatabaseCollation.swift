...
public final class DatabaseCollation: Identifiable, Sendable {
  ...
  public convenience init(
    _ name: String, function: @escaping @Sendable (String, String) -> ComparisonResult
  ) {
    self.init(
      name,
      xCompare: { (length1, buffer1, length2, buffer2) in
        let string1 = String(
          decoding: UnsafeRawBufferPointer(start: buffer1, count: Int(length1)), as: UTF8.self)
        let string2 = String(
          decoding: UnsafeRawBufferPointer(start: buffer2, count: Int(length2)), as: UTF8.self)
