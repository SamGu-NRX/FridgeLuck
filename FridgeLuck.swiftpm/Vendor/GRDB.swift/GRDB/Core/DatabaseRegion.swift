...
extension DatabaseRegion {
  ...
  public func isModified(by event: DatabaseEvent) -> Bool {
    ...
    guard let tableRegion = tableRegions[CaseInsensitiveIdentifier(rawValue: event.tableName)]
    else {
...
extension DatabaseRegion {
  ...
  static func union(_ regions: [any DatabaseRegionConvertible])
    -> @Sendable (Database) throws -> DatabaseRegion
  {
    return { db in
