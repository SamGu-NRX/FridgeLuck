...
extension Row {
  ...
  @inline(__always)
  @inlinable
  public subscript<Value: DatabaseValueConvertible & StatementColumnConvertible>(_ index: Int)
    -> Value
  {
    try! decode(Value.self, atIndex: index)
  }
  ...
  /// ```
  @inlinable
  public subscript<Value: DatabaseValueConvertible & StatementColumnConvertible>(
    _ columnName: String
  ) -> Value {
    try! decode(Value.self, forColumn: columnName)
  }
  ...
  /// - warning: The `Data` argument to the body must not be stored or used
  ///   outside of the lifetime of the call to the closure.
  public func withUnsafeData<T>(at column: some ColumnExpression, _ body: (Data?) throws -> T)
    throws -> T
  {
...
public final class RowCursor: DatabaseCursor {
  ...
  init(statement: Statement, arguments: StatementArguments? = nil, adapter: (any RowAdapter)? = nil)
    throws
  {
...
extension Row {
  ...
  public static func fetchCursor(_ db: Database, _ request: some FetchRequest) throws -> RowCursor {
    ...
    precondition(
      request.supplementaryFetch == nil, "Not implemented: fetchCursor with supplementary fetch")
...
extension Row {
  ...
  public struct ScopesView {
    ...
    init(
      row: Row, scopes: [String: any _LayoutedRowAdapter], prefetchedRows: Row.PrefetchedRowsView
    ) {
      self.row = row
      self.scopes = scopes
...
extension OrderedDictionary<String, Row.Prefetch> {
  fileprivate mutating func setRows(_ rows: [Row], forKeyPath keyPath: [String]) {
    ...
    if keyPath.isEmpty {
      ...
      self[key, default: Row.Prefetch(rows: nil, prefetches: [:])].prefetches.setRows(
        rows, forKeyPath: keyPath)
    }
  }
}
