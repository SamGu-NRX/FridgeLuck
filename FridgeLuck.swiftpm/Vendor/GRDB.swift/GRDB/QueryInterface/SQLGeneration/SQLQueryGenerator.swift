...
struct SQLQueryGenerator: Refinable {
  ...
  private func optimizedSelectedRegion(_ db: Database, _ selectedRegion: DatabaseRegion) throws
    -> DatabaseRegion
  {
  ...
  private func makeTrivialDeleteStatement(_ db: Database, selection: [any SQLSelectable]) throws
    -> Statement
  {
    let tableName = relation.source.tableName
  ...
  private func grouping(_ db: Database) throws -> GroupingInfo {
    ...
    // SELECT * FROM player
    guard let groupExpressions = try relation.groupPromise?.resolve(db),
      groupExpressions.isEmpty == false
    else {
      return .none
    }
    ...
    for expression in groupExpressions {
      guard
        let column = try expression.column(db, for: relation.source.alias, acceptsBijection: true)
      else {
...
private struct SQLQualifiedRelation: Sendable {
    ...
    -> (adapter: any RowAdapter, endIndex: Int)?
  {
    ...
    for (key, join) in joins {
      if let (joinAdapter, joinEndIndex) =
        try join
        .relation
...
private struct SQLQualifiedJoin: Refinable {
    ...
    throws -> String
  {
    ...
    if let expression = try condition.joinExpression(
      context.db,
      leftAlias: leftAlias,
