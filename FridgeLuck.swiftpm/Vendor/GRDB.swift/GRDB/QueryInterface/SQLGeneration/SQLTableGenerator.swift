  ...
  func sql(_ db: Database) throws -> String {
    ...
    do {
      ...
      do {
        var items: [String] = []
        try items.append(
          contentsOf: columnGenerators.map {
        ...
        if let constraint = primaryKeyConstraint {
          ...
          chunks.append(
            "(\(constraint.columns.map(\.quotedDatabaseIdentifier).joined(separator: ", ")))")
  ...
  init(_ db: Database, table: TableDefinition) throws {
      ...
      throws -> SQLTableGenerator.KeyConstraint
    {
      try SQLTableGenerator.KeyConstraint(
        columns: constraint.components.flatMap { component -> [String] in
          switch component {
          case .columnName(let columnName):
            return [columnName]
          case .columnDefinition(let column):
            return [column.name]
          case .foreignKeyDefinition(let foreignKey):
    ...
    if let primaryKeyConstraint = table.primaryKeyConstraint {
      ...
      for component in table.columnComponents {
        if case .columnDefinition(let column) = component, column.primaryKey != nil {
    ...
    for component in table.columnComponents {
      ...
      case .foreignKeyConstraint(let constraint):
        foreignKeyConstraints.append(constraint)
      }
    }

    for columnGenerator in columnGenerators {
      if case .columnDefinition(let column) = columnGenerator,
