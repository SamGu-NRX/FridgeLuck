...
extension TableRecord {
  ...
  static var defaultDatabaseTableName: String {
    ...
    let typeName = "\(Self.self)".replacingOccurrences(
      of: "(.)\\b.*$", with: "$1", options: [.regularExpression])
    let initial = typeName.replacingOccurrences(
    ...
    defaultDatabaseTableNameCache.setObject(
      tableName as NSString, forKey: "\(Self.self)" as NSString)
    return tableName
  }
...
extension TableRecord {
  ...
  public static func exists(_ db: Database, key: [String: (any DatabaseValueConvertible)?]) throws
    -> Bool
  {
    try !filter(key: key).isEmpty(db)
  }
...
extension TableRecord {
  ...
  /// - returns: The number of deleted records.
  @discardableResult
  public static func deleteAll(_ db: Database, keys: [[String: (any DatabaseValueConvertible)?]])
    throws -> Int
  {
    if keys.isEmpty {
      // Avoid hitting the database
  ...
  /// - returns: Whether a record was deleted.
  @discardableResult
  public static func deleteOne(_ db: Database, key: [String: (any DatabaseValueConvertible)?])
    throws -> Bool
  {
...
extension RecordError: CustomStringConvertible {
  public var description: String {
    switch self {
    case .recordNotFound(let databaseTableName, let key):
...
extension TableRecord where Self: EncodableRecord {
  ...
  public func recordNotFound(_ db: Database) -> any Error {
    do {
      ...
      let key = Dictionary(
        uniqueKeysWithValues: primaryKey.columns.map {
