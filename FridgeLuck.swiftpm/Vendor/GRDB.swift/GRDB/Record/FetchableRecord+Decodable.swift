...
public class FetchableRecordDecoder {
  ...
  public func decode<T: FetchableRecord & Decodable>(_ type: T.Type, from row: Row) throws -> T {
    let decoder = _RowDecoder<T>(
      row: row, codingPath: [], columnDecodingStrategy: T.databaseColumnDecodingStrategy)
...
private struct _RowDecoder<R: FetchableRecord>: Decoder {
  ...
  class KeyedContainer<Key: CodingKey>: KeyedDecodingContainerProtocol {
    ...
    func decode(_ type: Bool.Type, forKey key: Key) throws -> Bool {
      try decoder.row.decode(forColumn: decodeColumn(forKey: key))
    }
        ...
        let index = row.index(forColumn: column)
      {
        ...
        if type == Data.self {
          ...
          return try R
            ...
            .decodeIfPresent(fromRow: row, atUncheckedIndex: index) as! T?
        } else if let type = T.self
          as? any (DatabaseValueConvertible & StatementColumnConvertible).Type
        {
    ...
    func nestedUnkeyedContainer(forKey key: Key) throws -> UnkeyedDecodingContainer {
      throw DecodingError.typeMismatch(
        ...
        DecodingError.Context(
          codingPath: codingPath, debugDescription: "unkeyed decoding is not supported"))
    }
    ...
    where T: Decodable {
      if let type = T.self as? any FetchableRecord.Type {
        ...
        let decoder = _RowDecoder(
          row: row, codingPath: codingPath, columnDecodingStrategy: .useDefaultKeys)
        return try T(from: decoder)
      }
    }
