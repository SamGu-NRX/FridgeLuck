  ...
  struct OnDemandFuture<Output, Failure: Error>: Publisher {
    ...
    where S: Subscriber & GRDBSendableMetatype, Failure == S.Failure, Output == S.Input {
  ...
  where Downstream: Subscriber & GRDBSendableMetatype {
    ...
    func request(_ demand: Subscribers.Demand) {
      lock.synchronized {
        switch state {
        case .waitingForDemand(let downstream, let attemptToFulfill):
          guard demand > 0 else {
            return
          }
    ...
    private func receive(_ value: Downstream.Input) {
      lock.synchronized { sideEffect in
        switch state {
        case .waitingForFulfillment(let downstream):
          state = .finished
          sideEffect = {
