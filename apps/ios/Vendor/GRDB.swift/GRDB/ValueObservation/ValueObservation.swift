...
public struct AsyncValueObservation<Element: Sendable>: AsyncSequence, Sendable {
  public typealias BufferingPolicy = AsyncThrowingStream<Element, Error>.Continuation
    .BufferingPolicy
  ...
  public func makeAsyncIterator() -> Iterator {
    ...
    let stream = AsyncThrowingStream(Element.self, bufferingPolicy: bufferingPolicy) {
      continuation in
      cancellable = start(
        // onError
        { error in
  ...
  extension DatabasePublishers {
    ...
    public struct Value<Output>: Publisher {
      ...
      where
        S: Subscriber & GRDBSendableMetatype,
        Failure == S.Failure,
        Output == S.Input
      {
    ...
    where
      Downstream: Subscriber & GRDBSendableMetatype,
      Downstream.Failure == Error
    {
      ...
      func request(_ demand: Subscribers.Demand) {
        lock.synchronized {
          switch state {
          case .waitingForDemand(let info):
            guard demand > 0 else {
              return
            }
            state = .observing(
