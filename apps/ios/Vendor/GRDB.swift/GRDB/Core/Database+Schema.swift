...
extension Database {
  ...
  private func schemaIdentifier(named schemaName: String) throws -> DatabaseSchemaID {
    ...
    func table(for table: DatabaseObjectID) throws -> TableInfo? {
      ...
      GRDBPrecondition(sqlite3_libversion_number() >= 3_037_000, "SQLite 3.37+ required")
  ...
  private func _table(for table: DatabaseObjectID) throws -> TableInfo? {
    assert(sqlite3_libversion_number() >= 3_037_000, "SQLite 3.37+ required")
    SchedulingWatchdog.preconditionValidQueue(self)
    ...
    guard
      let tableInfo =
        ...
        .fetchOne(
          self,
          sql: "PRAGMA \(table.schemaID.name).table_list(\(table.name.quotedDatabaseIdentifier))")
    else {
  ...
  private func exists(type: SchemaObjectType, name: String, in schemaID: DatabaseSchemaID) throws
    -> Bool
  {
  ...
  private func indexes(on table: DatabaseObjectID) throws -> [IndexInfo]? {
    ...
    let indexes =
      ...
      .fetchAll(
        self,
        sql: "PRAGMA \(table.schemaID.name).index_list(\(table.name.quotedDatabaseIdentifier))"
      )
  ...
  /// with this name exists in the main or temp schema, or in an attached
  /// database.
  public func foreignKeys(on tableName: String, in schemaName: String? = nil) throws
    -> [ForeignKeyInfo]
  {
    if let schemaName {
      return try introspect(
        tableNamed: tableName, inSchemaNamed: schemaName, using: foreignKeys(on:))
    }
  ...
  private func foreignKeys(on table: DatabaseObjectID) throws -> [ForeignKeyInfo]? {
    ...
    let foreignKeys = try rawForeignKeys.map {
      (id, destinationTable, columnMapping) -> ForeignKeyInfo in
      let orderedMapping =
        columnMapping
        .sorted { $0.seq < $1.seq }
  ...
  private func foreignKeyViolations(in table: DatabaseObjectID) throws -> RecordCursor<
    ForeignKeyViolation
  > {
    try ForeignKeyViolation.fetchCursor(
      self,
      sql: """
...
extension Database {
  ...
  func fetchPrimitiveColumns(in table: DatabaseObjectID) throws -> [ColumnInfo]? {
    ...
    if sqlite3_libversion_number() < 3_026_000 {
      columnInfoQuery =
        "PRAGMA \(table.schemaID.name).table_info(\(table.name.quotedDatabaseIdentifier))"
    } else {
      // Use PRAGMA table_xinfo so that we can load generated columns
      columnInfoQuery =
        "PRAGMA \(table.schemaID.name).table_xinfo(\(table.name.quotedDatabaseIdentifier))"
    }
    let columns =
      try ColumnInfo
...
public struct DatabaseSchemaID: Hashable, Sendable {
  ...
  public var schemaTableName: String {
    ...
    case .attached(let name): return "\(name).sqlite_master"
    }
  }
...
public struct ForeignKeyViolation: Sendable {
  ...
  public func failureDescription(_ db: Database) throws -> String {
    ...
    let originRow = try originRowID.flatMap { rowid in
      try Row.fetchOne(
        db, sql: "SELECT * FROM \(originTable.quotedDatabaseIdentifier) WHERE rowid = \(rowid)")
    }
...
public struct PrimaryKeyInfo: Sendable {
  ...
  public var columns: [String] {
    ...
    case .regular(let columnInfos, tableHasRowID: _):
  ...
  var tableHasRowID: Bool {
    ...
    case .rowID:
      return true
    case .regular(columnInfos: _, let tableHasRowID):
...
extension SchemaInfo {
  ...
  init(_ db: Database, schemaTableName: String) throws {
    objects = try SchemaObject.fetchSet(
      db,
      sql: """
