...
class DatabaseObservationBroker {
  ...
  func add(
    transactionObserver: some TransactionObserver, extent: Database.TransactionObservationExtent
  ) {
    transactionObservations.append(
      TransactionObservation(observer: transactionObserver, extent: extent))
  }
  ...
  func statementDidExecute(_ statement: Statement) throws {
    ...
    private func databaseWillChange(with event: DatabasePreUpdateEvent) {
      ...
      if savepointStack.isEmpty {
        ...
        where statementObservation.tracksEvent(event) {
          statementObservation.transactionObservation.databaseWillChange(with: event)
        }
      } else {
        // Buffer
        savepointStack.eventsBuffer.append(
          (event: event.copy(), statementObservations: statementObservations))
      }
    }
  #endif
  ...
  private func notifyBufferedEvents() {
    ...
    for (event, statementObservations) in eventsBuffer {
      assert(
        statementObservations.isEmpty || !database.isReadOnly,
        "Read-only transactions are not notified")
      for statementObservation in statementObservations
      where statementObservation.tracksEvent(event) {
  ...
  func installCommitAndRollbackHooks() {
    ...
    sqlite3_commit_hook(
      ...
      { brokerPointer in
        let broker = Unmanaged<DatabaseObservationBroker>.fromOpaque(brokerPointer!)
          .takeUnretainedValue()
        do {
  ...
  private func installUpdateHook() {
    ...
    sqlite3_update_hook(
      ...
      sqlite3_preupdate_hook(
        ...
        {
          (
            brokerPointer, databaseConnection, updateKind, databaseNameCString, tableNameCString,
            initialRowID, finalRowID
          ) in
          let broker = Unmanaged<DatabaseObservationBroker>.fromOpaque(brokerPointer!)
            .takeUnretainedValue()
...
struct StatementObservation {
  ...
  init(
    transactionObservation: TransactionObservation, trackingEvents predicate: DatabaseEventPredicate
  ) {
...
extension DatabaseEventKind {
  ...
  public var tableName: String {
    switch self {
    case .insert(let tableName): return tableName
    case .delete(let tableName): return tableName
    case .update(let tableName, columnNames: _): return tableName
    }
  }
}
...
private struct CopiedDatabaseEventImpl: DatabaseEventImpl {
  ...
  public struct DatabasePreUpdateEvent {
    ...
    fileprivate init(
      kind: Kind, initialRowID: Int64?, finalRowID: Int64?, impl: any DatabasePreUpdateEventImpl
    ) {
      self.kind = kind
  ...
  private struct MetalDatabasePreUpdateEventImpl: DatabasePreUpdateEventImpl {
    ...
    private func preupdate_getValues(
      ...
      sqlite_func: (_ connection: SQLiteConnection, _ column: CInt, _ value: inout SQLiteValue?) ->
        CInt
    )
      -> [DatabaseValue]?
    {
...
enum DatabaseEventPredicate {
  ...
  func callAsFunction(_ event: some DatabaseEventProtocol) -> Bool {
    ...
    case .matching(let observedEventKinds, let authorizerEventKinds):
...
class SavepointStack {
  ...
  var eventsBuffer:
    [(event: any DatabaseEventProtocol, statementObservations: [StatementObservation])] = []
