...
public final class Database: CustomStringConvertible, CustomDebugStringConvertible {
  ...
  init(
    ...
    self.sqliteConnection = try Database.openConnection(
      path: path, flags: configuration.SQLiteOpenFlags)
  ...
  private func activateExtendedCodes() throws {
    if (configuration.SQLiteOpenFlags & 0x0200_0000 /* SQLITE_OPEN_EXRESCODE */) != 0 {
      // Nothing to do
      return
    }
    ...
    guard code == SQLITE_OK else {
      throw DatabaseError(
        resultCode: code, message: String(cString: sqlite3_errmsg(sqliteConnection)))
    }
  }
    ...
    guard code == SQLITE_OK else {
      ...
      if let log = Self.logError {
        if ResultCode(rawValue: code).primaryResultCode == .SQLITE_BUSY {
          ...
          while stmt != nil {
            log(
              ResultCode(rawValue: code),
              "unfinalized statement: \(String(cString: sqlite3_sql(stmt)))")
  ...
  public func add(collation: DatabaseCollation) {
    ...
    let code = sqlite3_create_collation_v2(
      ...
      { (collationPointer, length1, buffer1, length2, buffer2) in
        let collation = Unmanaged<DatabaseCollation>.fromOpaque(collationPointer!)
          .takeUnretainedValue()
        return collation.xCompare(
          length1, buffer1.unsafelyUnwrapped, length2, buffer2.unsafelyUnwrapped)
      }, nil)
    guard code == SQLITE_OK else {
  ...
  public func trace(options: TracingOptions = .statement, _ trace: ((TraceEvent) -> Void)? = nil) {
    ...
    let dbPointer = Unmanaged.passUnretained(self).toOpaque()
    sqlite3_trace_v2(
      sqliteConnection, CUnsignedInt(bitPattern: options.rawValue),
      { (mask, dbPointer, p, x) in
        let db = Unmanaged<Database>.fromOpaque(dbPointer!).takeUnretainedValue()
  ...
  private func trace_v2(
    ...
    _ sqlite3_expanded_sql:
      @escaping @convention(c) (OpaquePointer?) -> UnsafeMutablePointer<CChar>?
  ) {
  ...
  @discardableResult
  public func checkpoint(_ kind: Database.CheckpointMode = .passive, on dbName: String? = "main")
    throws
    -> (walFrameCount: Int, checkpointedFrameCount: Int)
  {
    ...
    let code = sqlite3_wal_checkpoint_v2(
      sqliteConnection, dbName, kind.rawValue,
      &walFrameCount, &checkpointedFrameCount)
    switch code {
    case SQLITE_OK:
      return (
        walFrameCount: Int(walFrameCount), checkpointedFrameCount: Int(checkpointedFrameCount)
      )
    case SQLITE_MISUSE:
      throw DatabaseError(resultCode: code)
    default:
  ...
  public func inTransaction(
    _ kind: TransactionKind? = nil, _ operations: () throws -> TransactionCompletion
  ) throws {
    // Begin transaction
  ...
  {
    guard
      let backup = sqlite3_backup_init(destDb.sqliteConnection, "main", sqliteConnection, "main")
    else {
...
extension Database {
  ...
  func countChanges<T>(_ count: inout Int, forTable tableName: String, updates: () throws -> T)
    throws -> T
  {
...
extension Database {
  ...
  public typealias LogErrorFunction =
    @Sendable (_ resultCode: ResultCode, _ message: String) -> Void
