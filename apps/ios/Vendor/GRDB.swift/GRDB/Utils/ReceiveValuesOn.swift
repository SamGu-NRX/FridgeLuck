    ...
    Upstream.Output == Downstream.Input
  {
    ...
    func request(_ demand: Subscribers.Demand) {
      lock.synchronized { sideEffect in
        switch state {
        case .waitingForRequest(let upstream, let target):
          state = .waitingForSubscription(target, demand)
          sideEffect = {
            upstream.receive(subscriber: self)
          }

        case .waitingForSubscription(let target, let currentDemand):
          state = .waitingForSubscription(target, demand + currentDemand)

        case .subscribed(_, let subcription):
          sideEffect = {
  ...
  extension Publisher {
    ...
    func receiveValues<S: Scheduler>(on scheduler: S, options: S.SchedulerOptions? = nil)
      -> ReceiveValuesOn<Self, S>
    {
