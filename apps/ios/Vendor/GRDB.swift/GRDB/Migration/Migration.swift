...
struct Migration: Sendable {
  ...
  func deleteMergedIdentifiers(_ db: Database) throws {
    if mergedIdentifiers.isEmpty == false {
      try db.execute(
        literal: "DELETE FROM grdb_migrations WHERE identifier IN \(mergedIdentifiers)")
    }
  }

  private func runWithImmediateForeignKeysChecks(_ db: Database, mergedIdentifiers: Set<String>)
    throws
  {
  ...
  private func runWithDisabledForeignKeysChecks(_ db: Database, mergedIdentifiers: Set<String>)
    throws
  {
  ...
  private func runWithDeferredForeignKeysChecks(_ db: Database, mergedIdentifiers: Set<String>)
    throws
  {
