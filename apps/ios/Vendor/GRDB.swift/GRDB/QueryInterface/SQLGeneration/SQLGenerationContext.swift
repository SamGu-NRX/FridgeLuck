...
final class SQLGenerationContext {
  ...
  var db: Database {
    switch parent {
    case .none(let db, argumentsSink: _): return db
    case .context(let context): return context.db
    }
  }
  ...
  private var argumentsSink: StatementArgumentsSink {
    switch parent {
    case .none(db: _, let argumentsSink): return argumentsSink
    case .context(let context): return context.argumentsSink
    }
  }
  ...
  func columnCount(
    ...
    case .none(let db, argumentsSink: _):
      if excludedColumns.isEmpty {
...
extension [TableAliasBase] {
  ...
  fileprivate var resolvedNames: [TableAliasBase: String] {
    ...
    //          .including(required: Book.author.aliased(alias)...)
    GRDBPrecondition(
      count == Set(self).count, "A TableAlias most not be used to refer to multiple tables")
