...
public class TableAliasBase: @unchecked Sendable {
  ...
  var tableName: String {
    ...
    case .table(let tableName, userName: _):
      return tableName
    case .proxy(let base):
  ...
  private var userName: String? {
    ...
    case .undefined(let userName):
      return userName
    case .table(tableName: _, let userName):
      return userName
    case .proxy(let base):
  ...
  func becomeProxy(of base: TableAliasBase) {
    ...
    case .table(let tableName, let userName):
  ...
  func merged(with other: TableAliasBase) -> TableAliasBase? {
    ...
    case (
      .table(tableName: let tableName, userName: let userName),
      .table(tableName: let otherTableName, userName: let otherUserName)
    ):
...
extension TableAliasBase {
  ...
  /// ```
  public subscript(_ expression: some SQLSpecificExpressible & SQLSelectable & SQLOrderingTerm)
    -> SQLExpression
  {
...
extension TableAlias where RowDecoder: TableRecord {
  ...
  where T: SQLSpecificExpressible & SQLSelectable & SQLOrderingTerm {
    self[RowDecoder.databaseComponents[keyPath: keyPath]]
  }
  ...
  where T: SQLJSONExpressible & SQLSpecificExpressible & SQLSelectable & SQLOrderingTerm {
    self[RowDecoder.databaseComponents[keyPath: keyPath]]
  }
