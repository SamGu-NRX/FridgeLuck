...
public struct SQLExpression: Sendable {
  ...
  private enum Impl {
    ...
    indirect case escapableBinary(
      EscapableBinaryOperator, SQLExpression, SQLExpression, escape: SQLExpression?)
    ...
    func qualified(with alias: TableAliasBase) -> Impl {
      ...
      case .rowValue(let expressions):
        return .rowValue(expressions.map { $0.qualified(with: alias) })

      case .literal(let sqlLiteral):
        return .literal(sqlLiteral.qualified(with: alias))

      case .cast(let expression, let storageClass):
        return .cast(expression.qualified(with: alias), storageClass)

      case .between(let expression, let lowerBound, let upperBound, let isNegated):
      ...
      case .escapableBinary(let op, let lhs, let rhs, let escape):
      ...
      case .associativeBinary(let op, let expressions):
        return .associativeBinary(op, expressions.map { $0.qualified(with: alias) })

      case .in(let expression, let collection, let isNegated):
        return .in(
...
extension SQLExpression {
  ...
  static func between(
    ...
    if case .collated(let expression, let collationName) = expression.impl {
  ...
  static func associativeBinary(_ op: AssociativeBinaryOperator, _ expressions: [SQLExpression])
    -> Self
  {
  ...
  static func `in`(
    _ expression: SQLExpression, _ collection: SQLCollection, isNegated: Bool = false
  ) -> Self {
...
extension SQLExpression {
  ...
  func qualified(with alias: TableAliasBase) -> Self {
    .init(
      impl: impl.qualified(with: alias), preferredJSONInterpretation: preferredJSONInterpretation)
  }
...
extension SQLExpression {
  ...
  ///   returns `score`.
  func column(_ db: Database, for alias: TableAliasBase, acceptsBijection: Bool = false) throws
    -> String?
  {
    switch impl {
    case .qualifiedColumn(let name, let a):
      if alias == a {
        return name
      } else {
        return nil
    ...
    case .collated(let expression, _):
      return try expression.column(db, for: alias, acceptsBijection: acceptsBijection)

    case .simpleFunction(let invocation) where acceptsBijection:
      return try invocation.column(db, for: alias)

    case .qualifiedFastPrimaryKey(let a):
      if alias == a {
  ...
  func sql(_ context: SQLGenerationContext, wrappedInParenthesis: Bool = false) throws -> String {
    ...
    case .qualifiedColumn(let name, let alias):
    ...
    case .databaseValue(let dbValue):
      if dbValue.isNull {
        // fast path for NULL
        return "NULL"
    ...
    case .cast(let expression, let storageClass):
      return try
        "CAST(\(expression.sql(context, wrappedInParenthesis: false)) AS \(storageClass.rawValue))"
    ...
    case .exists(let subquery, let isNegated):
      var resultSQL = try """
    ...
    case .tableMatch(let alias, let expression):
      var resultSQL = try """
    ...
    case .qualifiedFastPrimaryKey(let alias):
    ...
    case .qualifiedExists(let alias, let isNegated):
  ...
  func identifyingColums(_ db: Database, for alias: TableAliasBase) throws -> Set<String> {
    ...
    case .rowValue(let expressions):
      assert(!expressions.isEmpty)
      return try expressions.reduce(into: []) {
        try $0.formUnion($1.identifyingColums(db, for: alias))
      }
  ...
  func identifyingRowIDs(_ db: Database, for alias: TableAliasBase) throws -> Set<Int64>? {
    ...
    case .in(let expression, let collection, isNegated: false):
        ...
        try db.columnIsRowID(column, of: alias.tableName)
      {
        return Set(
          expressions.compactMap { expression in
            if case .databaseValue(let dbValue) = expression.impl {
  ...
  var isConstantInRequest: Bool {
    ...
    case .rowValue(let expressions),
      .associativeBinary(_, let expressions):
    ...
    case .between(
      let expression, let lowerBound, let upperBound,
      isNegated: _):
      return expression.isConstantInRequest
    ...
    case .binary(_, let lhs, let rhs),
      .compare(_, let lhs, let rhs):
      return lhs.isConstantInRequest && rhs.isConstantInRequest

    case .in(let expression, let collection, isNegated: _):
    ...
    case .unary(_, let expression),
      .not(let expression),
      .isEmpty(let expression, isNegated: _),
      .collated(let expression, _):
      return expression.isConstantInRequest
  ...
  var isAggregate: Bool {
    ...
    case .between(let expression, lowerBound: _, upperBound: _, isNegated: _),
      .unary(_, let expression),
      .not(let expression),
      .collated(let expression, _),
      .isEmpty(let expression, isNegated: _):
  ...
  var isJSONValue: Bool {
    ...
    var jsonBuilderExpression: SQLExpression {
      ...
      case .jsonValue:
        if isJSONValue {
          return self
        } else if Database.sqliteLibVersionNumber >= 3_045_000 {
