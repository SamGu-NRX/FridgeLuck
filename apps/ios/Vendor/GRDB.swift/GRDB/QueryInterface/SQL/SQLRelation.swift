...
extension SQLRelation: Refinable {
  func selectWhenConnected(_ selection: @escaping @Sendable (Database) throws -> [SQLSelection])
    -> Self
  {
    with {
  ...
  func annotatedWhenConnected(
    with selection: @escaping @Sendable (Database) throws -> [SQLSelection]
  ) -> Self {
    with {
  ...
  func filterWhenConnected(_ predicate: @escaping @Sendable (Database) throws -> SQLExpression)
    -> Self
  {
    with {
  ...
  func orderWhenConnected(_ orderings: @escaping @Sendable (Database) throws -> [SQLOrdering])
    -> Self
  {
    with {
  ...
  func withStableOrder() -> Self {
    with { relation in
      relation.ordering = relation.ordering.appending(
        Ordering(orderings: { [relation] db in
          do {
            // Order by primary key. Don't order by rowid because those are
            // not stable: rowids can change after a vacuum.
            return try db.primaryKey(source.tableName).columns.map {
              SQLExpression.column($0).sqlOrdering
            }
          } catch {
  ...
  func groupWhenConnected(_ expressions: @escaping @Sendable (Database) throws -> [SQLExpression])
    -> Self
  {
    with {
  ...
  func havingWhenConnected(_ predicate: @escaping @Sendable (Database) throws -> SQLExpression)
    -> Self
  {
    with {
...
extension SQLRelation {
  ...
  private func fetchTrivialCount(_ db: Database) throws -> Int {
    let countRequest: SQLRequest<Int> =
      "SELECT COUNT(*) FROM (\(SQLSubquery.relation(unordered())))"
...
extension JoinMapping {
  ...
  func joinExpression(
    ...
    guard let firstLeftRow = leftRows.first else {
      ...
      fatalError(
        "Provide at least one left row, or this method can't generate SQL that can be observed.")
    }
...
extension SQLRelation {
  ...
  func merged(with other: SQLRelation) -> Self? {
    ...
    case (nil, let promise?), (let promise?, nil):
...
extension SQLSource {
  ...
  func merged(with other: SQLSource) -> SQLSource? {
    ...
    case (let alias?, nil), (nil, let alias?):
      return SQLSource(tableName: tableName, alias: alias)
    case (let alias?, let otherAlias?):
...
extension SQLAssociationCondition {
  func merged(with other: SQLAssociationCondition) -> Self? {
    switch (self, other) {
    case (.foreignKey(let lhs), .foreignKey(let rhs)):
      if lhs == rhs {
        return self
      } else {
        // can't merge
    ...
    case (.expression, .expression(let rhs)):
...
extension SQLRelation.Child.Kind {
  ...
  func merged(with other: SQLRelation.Child.Kind) -> Self? {
    ...
    case (.all, .bridge),
      ...
      fatalError(
        "Not implemented: merging a direct association and an indirect one with including(all:)")
