...
extension EncodableRecord {
  ...
  public static func databaseDataEncodingStrategy(for column: String)
    -> DatabaseDataEncodingStrategy
  {
    .deferredToData
  }
...
extension EncodableRecord {
  ...
  public func databaseEquals(_ record: Self) -> Bool {
    do {
      return try PersistenceContainer(self).changesIterator(from: PersistenceContainer(record))
        .next() == nil
    } catch {
