...
where Self: TableRecord & Hashable & Identifiable, ID: DatabaseValueConvertible {
  /// Returns a set of records identified by their primary keys.
  ///
  /// For example:
  ///
...
extension FetchableRecord where Self: TableRecord {
  ...
  /// - returns: An array of records.
  /// - throws: A ``DatabaseError`` whenever an SQLite error occurs.
  public static func fetchAll(_ db: Database, keys: [[String: (any DatabaseValueConvertible)?]])
    throws -> [Self]
  {
    if keys.isEmpty {
      // Avoid hitting the database
  ...
  /// - returns: An optional record.
  /// - throws: A ``DatabaseError`` whenever an SQLite error occurs.
  public static func fetchOne(_ db: Database, key: [String: (any DatabaseValueConvertible)?]?)
    throws -> Self?
  {
    guard let key else {
      // Avoid hitting the database
  ...
  ///   ``RecordError/recordNotFound(databaseTableName:key:)`` if the record
  ///   does not exist in the database.
  public static func find(_ db: Database, key: [String: (any DatabaseValueConvertible)?]) throws
    -> Self
  {
...
extension FetchableRecord where Self: TableRecord & Hashable {
  ...
  /// - returns: A set of records.
  /// - throws: A ``DatabaseError`` whenever an SQLite error occurs.
  public static func fetchSet(_ db: Database, keys: [[String: (any DatabaseValueConvertible)?]])
    throws -> Set<Self>
  {
    if keys.isEmpty {
      // Avoid hitting the database
