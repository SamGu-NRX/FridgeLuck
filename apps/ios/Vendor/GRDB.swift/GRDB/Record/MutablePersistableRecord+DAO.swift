...
final class DAO<Record: MutablePersistableRecord> {
  ...
  init(_ db: Database, _ record: Record) throws {
    ...
    GRDBPrecondition(
      !persistenceContainer.isEmpty, "\(type(of: record)): invalid empty persistence container")
  }
...
extension UpdateQuery {
  ...
  var sql: String {
    ...
    return Self.cacheLock.withLock { cache in
      let updateSQL = updatedColumns.map { "\($0.quotedDatabaseIdentifier)=?" }.joined(
        separator: ", ")
      let whereSQL = conditionColumns.map { "\($0.quotedDatabaseIdentifier)=?" }.joined(
        separator: " AND ")
      let sql: String
      switch onConflict {
      case .abort:
